...
def NBP1():
    # 开发环境
    app = Application(backend='win32').start(dir)
    ...
    main.child_window(title="user", auto_id="txtAccount", control_type="Edit").set_text(guser) 
    main.child_window(title="password", auto_id="txtPassword", control_type="Edit").set_text(gpassword)
    ...
    if paneSet.child_window(title="P1", auto_id="rbtP1", control_type="RadioButton").exists(timeout = 3):
        pass

    paneSet.child_window(title="P1", auto_id="rbtP1", control_type="RadioButton").click()
    ...
    app2.window(title ='Form1').child_window(auto_id="WbsMain", control_type="Pane").capture_as_image().save(r'./picture/NBP1.png')
    ...
    return [r'./picture/NBP1.png',"NBP1.png"]
